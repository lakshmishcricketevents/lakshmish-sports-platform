...
export default function CricketMatches() {
    ...
    async function fetchMatches() {
      try {
        ...
        if (Array.isArray(data)) {
          setMatches(data.filter(m => m.sport === 'cricket'));
        }
      } catch (err) {
  ...
  const subLinks = [
    ...
    { href: '/cricket/matches', label: 'Matches', active: true },
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0B1020] text-white">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* Title */}
        <div>
          <h1 className="text-2xl font-black uppercase tracking-wider">Cricket Match Center</h1>
          <p className="text-xs text-dark-450 uppercase font-semibold mt-1">Real-time match scoring dashboard and fixtures</p>
        </div>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    {(isLive || isCompleted) && match.cricketState && (
                      <div className="mt-4 bg-dark-950/80 p-3 rounded-xl border border-dark-850 font-mono font-black">
                        ...
                        {match.tossText && (
                          <div className="text-[8px] text-dark-400 uppercase tracking-widest text-center mt-1 font-sans font-extrabold">{match.tossText}</div>
                        )}
                      </div>
                    )}
                  </div>
