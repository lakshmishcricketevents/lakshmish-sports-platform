...
import { Play, Calendar, Trophy, Users, Award, ChevronRight, Star, BarChart2, Activity } from 'lucide-react';
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (Array.isArray(resMatches)) setMatches(resMatches.filter(m => m.sport === 'cricket'));
        if (Array.isArray(resTournaments)) setTournaments(resTournaments.filter(t => t.sport === 'cricket'));
        ...
        if (Array.isArray(resPlayers)) setPlayers(resPlayers.filter(p => p.stats?.cricket || p.role === 'Batsman' || p.role === 'Bowler' || p.role === 'All-Rounder' || p.role === 'Wicketkeeper'));
      } catch (err) {
        console.error('Failed to fetch cricket dashboard data:', err);
      } finally {
        setLoading(false);
      }
    }
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-purple-500/10 p-8 sm:p-12 bg-gradient-to-br from-dark-900 via-dark-950 to-[#0e122b] shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl">
            <span className="text-[10px] font-black uppercase tracking-widest text-purple-400 bg-purple-500/15 border border-purple-500/25 px-2.5 py-1 rounded-full">
              Karnataka Premier Leagues
            </span>
            <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-white mt-4 uppercase leading-none">
              Karnataka's Premier <br />
              <span className="purple-gradient-text">Cricket Hub</span>
            </h1>
            <p className="text-xs sm:text-sm text-dark-300 mt-4 leading-relaxed">
              Track live scores, match rosters, and points tables across all local tournaments. Use our live scorecards with ball-by-ball analysis and real-time run rate calculations.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {[
            ...
            { label: 'Registered Teams', value: teams.length, icon: Users },
            { label: 'Active Leagues', value: tournaments.length, icon: Trophy },
            { label: 'Registered Players', value: players.length, icon: Award }
          ].map((stat, i) => {
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between border-b border-dark-850 pb-3">
              ...
              <Link href="/cricket/matches" className="text-[10px] font-black uppercase text-purple-400 hover:underline">
                View All Matches →
              </Link>
            </div>
            ...
            {loading ? (
              ...
              <div className="glass-panel text-center py-12 rounded-2xl border border-dark-850">
                ...
                <p className="text-[9px] text-dark-450 mt-1 uppercase font-semibold">Check back later for cricket fixture announcements</p>
              </div>
            ) : (
              <div className="space-y-4">
                    ...
                    {match.cricketState && (
                      <div className="mt-3 bg-dark-950/60 p-2 rounded-xl text-center border border-dark-850">
                        <div className="text-white text-sm font-black font-mono">
                          ...
                          <span className="text-[10px] text-dark-450 font-normal ml-1.5">({match.cricketState.overs}.{match.cricketState.balls} Ov)</span>
                        </div>
                      </div>
                    )}
                  </Link>
                ...
                {upcomingMatches.slice(0, 3).map((match) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3 border-b border-dark-850 pb-2">
                      <span className="text-purple-400 text-[9px] font-black uppercase tracking-wider">UPCOMING MATCH</span>
                      <span className="text-[9px] text-dark-450 font-bold uppercase">{match.date}</span>
                    </div>
          ...
          <div className="space-y-6">
            ...
            {loading ? (
              ...
              <p className="text-xs text-dark-450 italic py-4 text-center">No active cricket leagues found.</p>
            ) : (
                ...
                {tournaments.map((tourney) => (
                  <div key={tourney.id} className="bg-dark-900/40 border border-dark-850 p-4 rounded-xl flex items-center justify-between">
                    ...
                    <Link
                      ...
                      className="p-1.5 bg-purple-500/10 border border-purple-500/20 text-purple-450 hover:bg-purple-500/20 rounded-lg"
                    >
                ...
                <div key={player.id} className="glass-panel border border-purple-500/10 rounded-2xl p-4 bg-dark-900/40 flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-dark-400 font-bold uppercase mt-1">
                      Runs: {player.stats?.cricket?.runs || 0} • Wickets: {player.stats?.cricket?.wickets || 0}
                    </div>
                  </div>
                </div>
              ))}
