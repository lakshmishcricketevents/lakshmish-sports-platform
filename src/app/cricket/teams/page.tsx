...
export default function CricketTeams() {
    ...
    async function fetchTeamsData() {
      try {
        ...
        if (Array.isArray(resTournaments)) {
          filteredTournaments = resTournaments.filter(t => t.sport === 'cricket');
          setTournaments(filteredTournaments);
        }

        if (Array.isArray(resTeams)) {
          // A team is a cricket team if it's registered in a cricket tournament
          // or if it doesn't belong to any tournament but is not specifically a kabaddi team (like Golden Warriors, etc.)
          const cricketTeamIds = new Set(filteredTournaments.flatMap(t => t.teams));
          const filteredTeams = resTeams.filter(team => 
            cricketTeamIds.has(team.id) || 
            (!cricketTeamIds.has(team.id) && team.name !== 'Golden Warriors')
          );
  ...
  const subLinks = [
    ...
    { href: '/cricket/teams', label: 'Teams', active: true },
                ...
                <div key={team.id} className="glass-panel border border-purple-500/10 rounded-2xl p-6 sm:p-8 bg-dark-900/20 shadow-xl space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex items-center space-x-3 bg-dark-950/40 p-3.5 rounded-xl border border-dark-850">
                      ...
                      <div>
                        <div className="text-[9px] text-dark-450 font-extrabold uppercase tracking-wider">Captain</div>
                    ...
                    <div className="flex items-center space-x-3 bg-dark-950/40 p-3.5 rounded-xl border border-dark-850">
                      ...
                      <div>
                        <div className="text-[9px] text-dark-450 font-extrabold uppercase tracking-wider">Vice Captain</div>
