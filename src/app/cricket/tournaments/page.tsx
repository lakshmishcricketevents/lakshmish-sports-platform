...
export default function CricketTournaments() {
    ...
    async function fetchTournamentsData() {
      try {
        ...
        if (Array.isArray(data)) {
          setTournaments(data.filter(t => t.sport === 'cricket'));
        }
      } catch (err) {
  ...
  const subLinks = [
    ...
    { href: '/cricket/tournaments', label: 'Standings', active: true },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-[#0B1020] text-white">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        ...
        <div>
          ...
          <p className="text-xs text-dark-450 uppercase font-semibold mt-1">Tournament points tables and tie-breaker statistics</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="glass-panel text-center py-12 px-4 rounded-xl border border-purple-500/10">
            ...
            <p className="text-dark-300 font-bold text-xs uppercase tracking-widest">No Active Cricket Leagues Found</p>
          </div>
        ) : (
          <div className="space-y-10">
            {tournaments.map((tourney) => {
              // Sort points table by points (descending), then by NRR (descending)
                    ...
                    <table className="min-w-full divide-y divide-dark-850 text-left text-xs text-white">
                      <thead className="bg-dark-950/60 text-[9px] font-extrabold uppercase tracking-widest text-dark-400">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-center">Net Run Rate</th>
                        ...
                        {sortedPointsTable.map((row, index) => (
                          <tr key={row.teamId} className="hover:bg-dark-900/35 transition-colors">
                            ...
                            <td className={`px-4 py-3.5 text-center font-mono font-bold ${
                              (row.netRunRate || 0) >= 0 ? 'text-emerald-450' : 'text-red-450'
                            }`}>
                              {(row.netRunRate || 0) > 0 ? `+${row.netRunRate}` : row.netRunRate}
                            </td>
