...
export default function KabaddiPlayers() {
  const [players, setPlayers] = useState<Player[]>([]);
  const [teams, setTeams] = useState<Team[]>([]);
  const [loading, setLoading] = useState(true);
  const [roleFilter, setRoleFilter] = useState<'All' | 'Raider' | 'Defender' | 'All-Rounder (Kabaddi)'>('All');
    ...
    async function fetchPlayersData() {
      try {
        ...
        if (Array.isArray(resPlayers)) {
          // Filter to only include Kabaddi players
          const kabaddiPlayers = resPlayers.filter(p => 
            p.stats?.kabaddi || 
            ['Raider', 'Defender', 'All-Rounder (Kabaddi)'].includes(p.role)
          );
          setPlayers(kabaddiPlayers);
        }
  ...
  const subLinks = [
    ...
    { href: '/kabaddi/players', label: 'Players', active: true },
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-dark-450 uppercase font-semibold mt-1">Track raid points, tackles, super raids, and market valuations</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2 border-b border-dark-850 pb-4">
          {[
            ...
            { id: 'Raider', label: 'Raiders' },
            { id: 'Defender', label: 'Defenders' },
            { id: 'All-Rounder (Kabaddi)', label: 'All-Rounders' }
          ].map((role) => (
            <button
              key={role.id}
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredPlayers.map((player) => {
              ...
              const raidPoints = player.stats?.kabaddi?.raidPoints || 0;
              const tacklePoints = player.stats?.kabaddi?.tacklePoints || 0;
              const superRaids = player.stats?.kabaddi?.superRaids || 0;
              const superTackles = player.stats?.kabaddi?.superTackles || 0;
              const matchesPlayed = player.stats?.kabaddi?.matches || 0;
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {owningTeam ? (
                          ...
                          <span className="text-[8px] text-purple-455 font-bold uppercase block mt-0.5">
                            Unsold (Free Agent)
                          </span>
                        )}
                      </div>
                    ...
                    <div className="bg-dark-950/60 p-3 rounded-xl border border-dark-850 space-y-1.5 font-mono text-[10px]">
                      ...
                      <div className="flex justify-between border-t border-dark-900 pt-1.5">
                        <span className="text-dark-400 uppercase font-sans font-extrabold text-[8px]">Raid Points</span>
                        <span className="text-white font-black">{raidPoints}</span>
                      </div>
                      <div className="flex justify-between border-t border-dark-900 pt-1.5">
                        <span className="text-dark-400 uppercase font-sans font-extrabold text-[8px]">Tackle Points</span>
                        <span className="text-white font-black">{tacklePoints}</span>
                      </div>
                      <div className="flex justify-between border-t border-dark-900 pt-1.5">
                        <span className="text-dark-400 uppercase font-sans font-extrabold text-[8px]">Super Raids / Tackles</span>
                        <span className="text-white font-black">{superRaids} / {superTackles}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-dark-850 mt-4 pt-3 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <span className="text-[8px] text-dark-455 font-extrabold uppercase block leading-none">Base Value</span>
